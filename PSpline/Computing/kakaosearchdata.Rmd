...
# 데이터
- Y data : Y데이터의 경우 134주(2009년 1월 ~ 2018년 12월)동안의 한국에 입국한 국적별 외국인 수를 나타냈다.
 
- X data :  X데이터의 경우 134주 동안의 각 나라의 한국 원화 기준 환율을 나타낸 것이다. 나라는 한글 순으로 [가나, 가봉, … , 헝가리,호주, 홍콩] 174개국으로 구성된 국가 중 pop TOP100 국가를 추출했다. 그리고 시간의 흐름에 따라 데이터를 나열했다.


```{r message=FALSE, warning=FALSE, include=FALSE}
# Y data
obs_y <- fread("soltr_kakao_y.csv")

# obs_y$Month <- parse_date_time(obs_y$Month)
...
# X data
x_pop <- obs_y[1,-1] %>% t()
# x_pop <- fread("C:\\onedrive\\OneDrive - 성균관대학교\\1석사\\연구\\내 연구\\PSpline\\Computing\\x_info.csv")
# x_pop <- x_pop[,1:2]
# x_pop$Month <- parse_date_time(x_pop$Month, "ym")
# x_pop$Month <- as.Date(x_pop$Month, format="%Y-%m-%d")
# x_pop <- as.data.frame(x_pop)
# x_pop <- x_pop %>% mutate_if(is.character,parse_number)
```
