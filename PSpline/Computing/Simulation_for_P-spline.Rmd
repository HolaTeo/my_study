...
- Simulation은 True $\mu$(Example1, Example2)
- 가정한 모형(LM,GLM)
- 추정방법(Single, naive(smoothing spline), naive(p-spline), multileve(smoothing spline), multilevel(p-spline))를 기준으로 
20개의 case 으로 이루어짐.

- 각 case는 가정한 모형의 관측값이나 link function을 이용한 기대값들의 분포의 분산(2,4,8)을 세가지의 경우에 대해 simulatoin
- 각 knots에 대한 sample size는 $N_{min}$과 200 사이에서 동일한 확률로 선택되었는데 $N_{min}$은 50과 100 두 가지의 경우에 대해 simulation함.


- 총 simulation 결과는 20(20개의 case) $\times$ 3(분포의 분산) $\times$ 2($N_{min}$) = 120 가지 경우로 나타냄.
- 각 결과는 200번의 반복을 통해 구한 RMSE 값입니다. 또한 200번의 반복을 통해 RMSE의 SD도 구하였습니다.
...
- simulation setting으로 20개의 case에 대한 결과를 나타냅니다. 결과는 RMSE 값과 RMSE의 SD값 입니다.
...
lambda <- c(100, 10, 1, 0.1, 0.01, 0.001)
min_lam <- NULL
...
for(t in c(2,4,8)){
  ...
  for(n in c(50,100)){
    ...
    for(j in 1:200){
      ...
      min_lam <-  rbind(min_lam, lambda[which.min(GCV_vec)])
      EM_out <- main_EM_p(beta_hat_vec = unlist(betahat[,1]), V = diag(unlist(betahat[,2])), B = B, D = D, lambda = lambda[which.min(GCV_vec)])
