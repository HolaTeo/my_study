...
# 데이터
- Y data : Y데이터의 경우 120달(2009년 1월~ 2018년 12월)동안의 GDP Top 100 국가의 한국에 입국한 외국인 수

- X data :  X데이터의 경우 GDP Top 100 국가의 GDP데이터이다.

```{r message=FALSE, warning=FALSE, include=FALSE}
# Y data
obs_y <- fread("obs_y.csv")[-1,-1]


# X data
x_gdp <- fread("obs_y.csv")[1,-1]
```
...
goodness <- NULL
  for(m in 1:120){
    result1_out <- NULL
    result2_out <- NULL
    results1 <- glm(unlist(y_list[m]) ~ unlist(x_list), family = poisson, maxit=5000)
    results2 <- glm.nb(unlist(y_list[m]) ~ unlist(x_list), maxit=5000)
...
```{r}
x_list <- x_gdp %>% as.data.frame() %>% unlist() %>% as.list()
y_list <- obs_y %>% t() %>% as.data.frame()
...
```{r}
lambda <- seq(71.65,71.67, by=.0005)
GCV_vec <- NULL
...
```{r}
naive_out <- naive_ss_p(beta_hat_vec = unlist(beta_hat[,1]), B=GetBSpline(unlist(beta_hat[,1]), deg = 3, IK, EK),  D=GetDiffMatrix(dim(B)[1],2),lambda=99999999999999)
...
hat_all$Month <- parse_date_time(hat_all$Month, "ymd")
...
df2$Month <- parse_date_time(df2$Month, "ymd")
