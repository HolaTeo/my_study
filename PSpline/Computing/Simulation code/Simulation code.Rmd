...
Simulation은 
True $\mu$(Example1, Example2), 
가정한 모형(LM,GLM), 
추정방법(Single, naive(smoothing spline), naive(p-spline), multileve(smoothing spline), multilevel(p-spline))를 기준으로 
20개의 case 으로 이루어져 있습니다.

그리고 각 case은 가정한 모형의 관측값이나 link function을 이용한 기대값들의 분포의 분산을 2,4,8 세가지의 경우에 대해 simulatoin 하였고
각 knots에 대한 sample size는 $N_{min}$과 200 사이에서 동일한 확률로 선택되었는데 $N_{min}$은 50과 100 두 가지의 경우에 대해 simulation 하였습니다.

따라서 총 simulation 결과는 20(20개의 case) $\times$ 3(분포의 분산) $\times$ 2($N_{min}$) = 120개 이며, 각 결과는 200번의 반복을 통해 구한 RMSE 값입니다. 또한 200번의 반복을 통해 RMSE의 SD도 구하였습니다.

# Functions

Simulation에 쓰일 함수들이며 pspline을 이용한 multilevel 추정방법인 EM algorithm과 navive 추정방법 또한 함수로 만들었습니다.
...
앞으로 첨부할 20개의 chunck는 simulation setting에서 설명드린 20개의 case에 대한 결과이며 각 chunck를 실행하면 나오는 결과는 RMSE 값과 RMSE의 SD값 입니다.
...
RMSE_NAIVEP_LM_1 <- RMSE_NAIVE_LM_1
SD_NAIVEP_LM_1 <- SD_NAIVE_LM_1
...
RMSE_MULTIP_LM_1 <- RMSE_MULTI_LM_1
SD_MULTIP_LM_1 <- SD_MULTI_LM_1
...
RMSE_NAIVEP_GLM_1 <- RMSE_NAIVE_GLM_1
SD_NAIVEP_GLM_1 <- SD_NAIVE_GLM_1 
...
RMSE_MULTIP_GLM_1 <- RMSE_MULTI_GLM_1
SD_MULTIP_GLM_1 <- SD_MULTI_GLM_1 
...
RMSE_NAIVEP_LM_2 <- RMSE_NAIVE_LM_2
SD_NAIVEP_LM_2 <- SD_NAIVE_LM_2
...
RMSE_MULTIP_LM_2 <- RMSE_MULTI_LM_2
SD_MULTIP_LM_2  <- SD_MULTI_LM_2 
...
RMSE_NAIVEP_GLM_2 <- RMSE_NAIVE_GLM_2
SD_NAIVEP_GLM_2 <- SD_NAIVE_GLM_2
...
SD_MULTI_GLM_2 <- NULL
lambda <- c(1000, 100, 10, 1, 0.1, 0.01, 0.001, 0.0001, 0.00001)
