---
title: "Multilevel Pspline from Youtube Data"
...
# 데이터
- Y data : Y데이터의 경우 292행(2018년 12월 15일 6시~ 2019년 1월20일 15시) 3시간 간격의 Top 100 채널의 채널별 증가한 조회수를 나타냈다.
 
- X data :  기간동안 평균 구독자 수의 Top100 채널을 선별했다. 


```{r message=FALSE, warning=FALSE, include=FALSE}
# Y data
obs_y <- fread("C:/onedrive/OneDrive - 성균관대학교/1석사/연구/내 연구/PSpline/Computing/hour_youtube_view.csv")
z_month <- unlist(obs_y[-1,2]) %>% scale()

# X data
x_pop <- obs_y[1,3:102] %>% t()
```
...
y_list <- obs_y[2:293,3:102] 
y_list <- data.Normalization(y_list,type = "n1", normalization = "column") %>% as.data.frame() %>% t()
#multilevel
...
```{r}
lambda <- c(0.1,1,10,100,10^3,10^4,10^5,10^6,10^7)
GCV_vec <- NULL

for(i in 1:length(lambda)){
EM_out <- MultiEM_ps(x=z_month,beta_hat_vec = unlist(beta_hat[,1]), V = diag(unlist(beta_hat[,2])),lambda[i])
...
- lambda[which.min(GCV_vec)]을 실행할 때, 10^8이 나온다. 
...
#naive
GCV_vec <- NULL
lambda <- c(10^4,10^5,10^6,10^7,10^8,10^9)
for(i in 1:length(lambda)){
...
```{r}
lambda[which.min(GCV_vec)]
naive_out <- naive_ps(x=z_month,beta_hat_vec=unlist(beta_hat[,1]),lambda = lambda[which.min(GCV_vec)])
...
test_mon <- fread("C:/onedrive/OneDrive - 성균관대학교/1석사/연구/내 연구/PSpline/Computing/hour_youtube_view.csv")
test_mon <- test_mon[-1,1]

mu_z <- cbind(obs_y[-1,2],mu_z_multi) %>% as.data.frame
mu_z <- rename(mu_z, Time = V2)
...
df2 <- cbind(obs_y[-1,2],single_beta)
df2 <- rename(df2, Time =V2)
...
# gather함수 사용
df2 <- gather(df2[, c("Time",  "single_beta", "Naive", "Multi")],
             key = "Method", value = "mu_z", -Time)

g <- ggplot(df2) +
  geom_line(data= df2 %>% dplyr::filter(Method != "single_beta"),aes(x = Time, y = mu_z, color = Method, linetype = Method)) +
  geom_point(data=df2 %>% dplyr::filter(Method == "single_beta"), aes(x = Time, y = single_beta, color = "single_beta")) +
