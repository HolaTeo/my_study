...
```{r message=FALSE, warning=FALSE, include=FALSE}
# Y data
obs_y <- fread("홈트레이닝 검색어 분석/HT_kakao_search.csv")
...
#multilevel
  ...
  for(m in 1:139){
    result2_out <- NULL
    results2 <- glm(unlist(y_list[m]) ~ unlist(x_list), maxit=2000, family = poisson)
...
test_mon <- fread("홈트레이닝 검색어 분석/HT_kakao_search.csv")
