...
z_month <- unlist(obs_y[-1,1]) %>% scale()
...
- 그래서 10^8 근처에서 GCV벡터를 더 찾아보기로 한다.
 
```{r}
# lambda <- seq(277.18,277.2, by=.0001)
lambda <- seq(10^8,10^9,by = 10^8)
...
#naive
# GCV_vec <- NULL
# lambda <- c(0.0001,0.001,0.01,0.1,1,10,100,1000, 10^4, 10^5)
# for(i in 1:length(lambda)){
#   naive_out <- FitPSpline(x=z_month,
#                           y=unlist(beta_hat[,1]),
#                           lambda[i])
...
test_mon <- fread("soltr_kakao_y.csv")
