...
# 데이터
- Y data : Y데이터의 경우 28주(2009년 1월 ~ 2018년 12월)동안의 한국에 입국한 국적별 외국인 수를 나타냈다.
 
- X data :  X데이터의 경우 28주 동안의 각 나라의 한국 원화 기준 환율을 나타낸 것이다. 나라는 한글 순으로 [가나, 가봉, … , 헝가리,호주, 홍콩] 174개국으로 구성된 국가 중 GDP TOP100 국가를 추출했다. 그리고 시간의 흐름에 따라 데이터를 나열했다.


```{r message=FALSE, warning=FALSE, include=FALSE}
# Y data
obs_y <- fread("C:\\onedrive\\OneDrive - 성균관대학교\\1석사\\연구\\내 연구\\PSpline\\Computing\\y_yt_week.csv")
...
# X data
x_gdp <- fread("C:\\onedrive\\OneDrive - 성균관대학교\\1석사\\연구\\내 연구\\PSpline\\Computing\\x_info.csv")
x_gdp <- x_gdp[,1:2]
...
test_mon <- fread("C:\\onedrive\\OneDrive - 성균관대학교\\1석사\\연구\\내 연구\\PSpline\\Computing\\y_yt_week.csv")
test_mon <- test_mon[,1]
